...
import me.caneva20.CNVCore.CNVActions.Paramenters.ActionParameter;
...
//    private Map<String, Boolean> validations = new HashMap<>();
    ...
    public void process(Map<String, Object> params, String raw) {
        if (!validate(params)) {
            CNVCore.getMainLogger().errorConsole("Invalid action. Some required parameters are not defined");
            CNVCore.getMainLogger().errorConsole("RAW Action: <par>" + raw + "</par>");
            CNVCore.getMainLogger().errorConsole("---------------------(Details)-----------------------");
            getValidation(params).forEach((param, validation) -> {
                CNVCore.getMainLogger().errorConsole(
